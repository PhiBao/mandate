...
1. **Claim.** Member runs `/claim <wallet>` → the bot asks for a tiny USDC fee (a random ~$0.02 amount, 10-minute window) sent **from that wallet** to the treasury. The transfer proves key control — only the key holder can send from that address — and its block timestamp becomes `claimed_at`: the forward-only clock is provable onchain, no DB trust required. Zero-cost fallback: sign the exact EIP-191 message (or sign in MetaMask via the linked page) → `/claim <wallet> <sig>`. The claim binds `wallet ↔ Telegram user ↔ group` and is permanent. Transfer fees double as claim-spam protection; one transfer verifies exactly one claim.
...
- **Transfer-based claims (wallet op, primary path):** claim-fee USDC transfers prove key control and timestamp the claim at block time — e.g. [`0x93dc4d73…f2322c`](https://basescan.org/tx/0x93dc4d73843444d1b0477f99da21e1ccbbe6f3496ca825165e0c2a588f2322c8) (exact random-amount match, settled by the bot's onchain watcher). Code path: `src/mandate/transfer_claim.py`, settled in `src/mandate/bot.py` `tick()`.
